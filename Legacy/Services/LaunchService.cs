...
namespace Arcane_Launcher.Services
{
    public class LaunchService
    {
        ...
        {
            await Authentication.RefreshToken(Properties.Settings.Default.refresh_token);
            ...
            {
                //LaunchFortniteLauncher(ExchangeCode, GamePath, $"-epicapp=Fortnite -epicenv=Prod -EpicPortal -steamimportavailable -epicusername=\"{Properties.Settings.Default.displayName}\" -epicuserid={Properties.Settings.Default.account_id} -epiclocale=en -epicsandboxid=fn -named_pipe={Properties.Settings.Default.account_id}\\Fortnite -src=12 -pl=01");
                LaunchEAC(ExchangeCode, GamePath, $"-epicapp=Fortnite -epicenv=Prod -EpicPortal -steamimportavailable -epicusername={Properties.Settings.Default.displayName} -epicuserid={Properties.Settings.Default.account_id} -epiclocale=en -epicsandboxid=fn -named_pipe={Properties.Settings.Default.account_id}\\Fortnite -src=12 -pl=01 -fromfl=eaceos -caldera=eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9..yAKraAncgy2ph3RJL0ccCgw_md0To-dKcloXh4tuWVhUWGReztoQD2oqIOFIKdMDqC8ML_K6gq2mbDk3VU7Y1g -startup=cf1fcacc-f3df-4a50-b540-0f01a284d8ea");
            }
            else
            {
        ...
        public static void LaunchFortniteLauncher(string ExchangeCode, string GamePath, string Args)
        {
            if (File.Exists(Path.Combine(GamePath, "FortniteGame\\Binaries\\Win64\\", "FortniteLauncher.exe")))
            {
                LaunchService.FNLauncherProcess = new Process()
                {
                    StartInfo = new ProcessStartInfo()
                    {
                        Arguments = $"-obfuscationid=nvW5DdYxswjSr6lVKE-aEnigt3q6UQ -AUTH_LOGIN=unused -AUTH_PASSWORD={ExchangeCode} AUTH_TYPE=exchangecode " + Args,
